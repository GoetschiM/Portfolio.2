...
export function createAIScene(player: Player): ManagedScene {
  ...
  scene.background = BG;
  scene.fog = new THREE.Fog(BG, 12, 85);

  scene.add(new THREE.AmbientLight(0xffffff, 0.25));
  const rim = new THREE.DirectionalLight(0x7fb0ff, 0.55);
  ...
  const inputPos = new THREE.Vector3(-6, 1.2, -6);
  const agentPos = new THREE.Vector3(0, 1.5, -10);
  const outputPos = new THREE.Vector3(6, 1.2, -6);
  ...
  return {
    ...
    getCamera: () => {
      const base = new THREE.Vector3(7.8, 7.8, 10.8);
